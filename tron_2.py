...
BLOCK_SIZE = 10
SPEED = 30
TRAIL_LIMIT = 100  # Maximum trail length for each player
...
class Player:
    ...
    def move(self):
        if self.direction != (0, 0):
            ...
            # Limit the trail length to TRAIL_LIMIT
...
def customization_menu():
    ...
    while customizing:
        ...
        t3 = font.render('WASD: P1, Arrows: P2', True, WHITE)
...
def main():
    ...
    while running:
        ...
        # Player 1: WASD
        if keys[pg.K_a]: player1.change_direction((-BLOCK_SIZE, 0))
        if keys[pg.K_d]: player1.change_direction((BLOCK_SIZE, 0))
        if keys[pg.K_w]: player1.change_direction((0, -BLOCK_SIZE))
        if keys[pg.K_s]: player1.change_direction((0, BLOCK_SIZE))
        # Player 2: AROWS KEYS
        if keys[pg.K_LEFT]: player2.change_direction((-BLOCK_SIZE, 0))
        if keys[pg.K_RIGHT]: player2.change_direction((BLOCK_SIZE, 0))
        if keys[pg.K_UP]: player2.change_direction((0, -BLOCK_SIZE))
        if keys[pg.K_DOWN]: player2.change_direction((0, BLOCK_SIZE))
        if not game_over:
            screen.fill(BLACK)
